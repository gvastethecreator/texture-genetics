...
import { AppState, TextureType, BlendMode, GeometryType, ViewMode, BaseEffectType } from './core/types/types';
...
const p = (
  ...
  return {
    ...
    state: {
      textureType: type,
      geometry: overrides.geometry || GeometryType.PLANE,
...
export const PRESETS: Preset[] = [
  ...
  p(TextureType.MAGIC_CIRCLE, 'Fantasy RPG', 'Eldritch Summoning', {
    params: {
        scale: 0.9, intensity: 2.5, speed: 0.2, factor: 0.6,
        palette: [c('#090014'), c('#240038'), c('#4a0e6e'), c('#9d00ff'), c('#d485ff'), c('#ffffff'), x, x],
        p1: 0.8, // Complexity
    ...
    environment: { holographic: true, holoStrength: 0.5, particlesEnabled: true, particleCount: 300, particleSpeed: 0.2, fogEnabled: true, fogColor: '#1a0033', fogDensity: 0.08 }
  }),
  p(TextureType.FLUID_WARP, 'Fantasy RPG', 'Mana Potion', {
    params: {
        scale: 2.5, intensity: 1.2, speed: 0.4, factor: 0.7,
    ...
    environment: { roughness: 0.1, metalness: 0.2, lightIntensity: 1.5, bgEnabled: true, bgColor: '#000000' },
    normalMap: { enabled: true, strength: 2.0, smoothness: 0.8 }
  }),
  p(TextureType.VORONOI_ROCKS, 'Fantasy RPG', 'Dragon Scales', {
    params: {
        scale: 4.0, intensity: 1.0, factor: 0.3,
  ...
  p(TextureType.SPHERICAL_SPIRAL, 'Fantasy RPG', 'Holy Shield', {
    params: {
        scale: 1.0, intensity: 2.0, speed: 0.5,
    ...
    blending: { enabled: true, type: TextureType.HOLO_FOIL, mode: BlendMode.ADD, opacity: 0.4, scale: 2, factor: 0.5, intensity: 1 }
  }),
  ...
  p(TextureType.BINARY_MATRIX, 'Cyberpunk', 'Netrunner Dive', {
    params: {
        scale: 1.5, intensity: 1.8, speed: 1.2,
    ...
    postProcess: { bloom: true, bloomStrength: 1.5, pixelate: true, pixelDensity: 128, glitch: true, glitchStrength: 0.3 }
  }),
  p(TextureType.CIRCUIT, 'Cyberpunk', 'Motherboard', {
    params: { 
        scale: 5.0, intensity: 1.2, factor: 0.3, speed: 0.05,
  ...
  p(TextureType.CONE, 'Cyberpunk', 'Targeting Radar', {
    params: {
        scale: 1.0, intensity: 1.5, speed: 2.0, factor: 1.0,
    ...
    postProcess: { scanlines: true, scanlineIntensity: 0.4, crtDistortion: 0.1, bloom: true, bloomStrength: 1.0 },
    blending: { enabled: true, type: TextureType.HEXAGON_GRID, mode: BlendMode.ADD, opacity: 0.3, scale: 5.0, factor: 0.1, intensity: 1.0 }
  }),
  p(TextureType.SCANLINES, 'Cyberpunk', 'System Failure', {
    params: {
        scale: 10.0, intensity: 1.0, speed: 0.2,
    ...
    postProcess: { glitch: true, glitchStrength: 1.0, glitchSpeed: 3.0, chromaticAberration: 1.5, crtDistortion: 0.3, pixelate: true, pixelDensity: 256 }
  }),
  ...
  p(TextureType.ALIEN_BIOMASS, 'Biotech', 'Zerg Creep', {
    params: {
        scale: 1.2, intensity: 1.0, factor: 0.6, speed: 0.3,
        palette: [c('#1a0505'), c('#2b0000'), c('#4d0014'), c('#800020'), c('#ff3333'), c('#ff9999'), x, x],
        p1: 0.6, // Pulse
        p3: 0.5, // Veins
        p7: 0.8, // Wetness
        p13: 0.4 // Slime
    ...
    environment: { roughness: 0.1, metalness: 0.1, envType: 1, lightIntensity: 2.0, sceneVignette: true }
  }),
  p(TextureType.TRABECULUM, 'Biotech', 'Bone Marrow', {
    params: { 
        scale: 4.0, intensity: 1.0, factor: 0.45,
  ...
  p(TextureType.GYROID, 'Biotech', 'Alien Brain', {
    params: {
        scale: 3.0, intensity: 1.0, factor: 0.5, speed: 0.1,
  ...
  p(TextureType.COMPLEX_FIRE, 'Elements', 'Inferno', {
    params: {
        scale: 1.5, intensity: 1.3, factor: 0.5, speed: 0.8,
        palette: [c('#000000'), c('#1a0500'), c('#4d1a00'), c('#ff4400'), c('#ffcc00'), c('#ffffff'), x, x],
        p2: 0.6, // Sparks
        p3: 0.4, // Turbulence
        p7: 0.7, // Core Temp
        p4: 0.3  // Smoke Mix
    ...
    environment: { smokeEnabled: true, smokeColor: '#331100', smokeDensity: 0.8, particlesEnabled: true, particleSpeed: 1.5, particleCount: 200 }
  }),
  p(TextureType.FROST_PATTERN, 'Elements', 'Glacial Ice', {
    params: {
        scale: 1.5, intensity: 1.0, factor: 0.7, speed: 0.02,
        palette: [c('#001122'), c('#113355'), c('#336699'), c('#88aacc'), c('#ddeeff'), c('#ffffff'), x, x],
  ...
  p(TextureType.REALISTIC_CLOUDS, 'Elements', 'Storm Front', {
    params: {
        scale: 1.2, intensity: 1.0, factor: 0.6, speed: 0.3,
        palette: [c('#0a0a1a'), c('#16213e'), c('#303a52'), c('#4b5d67'), c('#909090'), c('#c0c0c0'), x, x],
        p1: 0.8, // Coverage
        p2: 0.5, // Absorb
        p3: 0.6, // Density
        p12: 0.4 // Layering
    },
    environment: { envType: 3, lightIntensity: 0.6, fogEnabled: true, fogDensity: 0.02, fogColor: '#16213e' }
  }),
  p(TextureType.CAUSTICS, 'Elements', 'Caribbean Pool', {
    params: {
        scale: 3.0, intensity: 1.2, speed: 0.6, factor: 0.5,
        palette: [c('#006994'), c('#0088bb'), c('#00aadd'), c('#44ccff'), c('#88eeff'), c('#ffffff'), x, x],
        p3: 0.2, // Chromatic Aberration in pattern
        p2: 0.8  // Sharpness
    },
    blending: { enabled: true, type: TextureType.MOSAIC, mode: BlendMode.MULTIPLY, opacity: 0.4, scale: 6.0, factor: 0.0, intensity: 1.0 },
    environment: { stageEnabled: true, stageColor: '#004466' }
  }),
  p(TextureType.MARBLE, 'Elements', 'Molten Rock', {
    params: {
        scale: 2.0, intensity: 1.2, speed: 0.1,
        palette: [c('#1a0000'), c('#330000'), c('#660000'), c('#aa2200'), c('#ff6600'), c('#ffff00'), x, x],
  ...
  p(TextureType.GRUNGE, 'Materials', 'Rusted Metal', {
    params: {
        scale: 2.5, intensity: 1.0, factor: 0.7,
        palette: [c('#2b1d0e'), c('#4e2a14'), c('#8b4513'), c('#a0522d'), c('#cd853f'), c('#888888'), x, x],
        p1: 0.6, // Dirt
        p2: 0.5, // Scratch
        p4: 0.6, // Erode
        p11: 0.4 // Seed
  ...
  p(TextureType.BRICKS, 'Materials', 'Dungeon Wall', {
    params: {
        scale: 3.5, intensity: 1.0, factor: 0.6,
  ...
  p(TextureType.WEAVE_KNIT, 'Materials', 'Chainmail', {
    params: {
        scale: 12.0, intensity: 1.2,
        palette: [c('#000000'), c('#222222'), c('#555555'), c('#888888'), c('#aaaaaa'), c('#ffffff'), x, x],
        p1: 0.8, // Mix
        p3: 0.25, // Width
        p9: 0.1, // Tension
        p12: 0.0 // Grain
  ...
  p(TextureType.WOOD, 'Materials', 'Polished Oak', {
    params: {
        scale: 2.5, intensity: 1.0, factor: 0.5,
  ...
  p(TextureType.MOSAIC, 'Materials', 'Bathroom Tiles', {
    params: {
        scale: 8.0, intensity: 1.0, factor: 0.2,
  ...
  p(TextureType.HYPNOTIC_RINGS, 'Abstract', 'Vertigo', {
    params: {
        scale: 3.0, intensity: 1.0, speed: 1.5,
  ...
  p(TextureType.SMOOTH_SWIRL, 'Abstract', 'Liquid Candy', {
    params: {
        scale: 1.8, intensity: 1.0, speed: 0.6,
  ...
  p(TextureType.MANDELBROT, 'Abstract', 'Deep Fractal', {
    params: {
        scale: 1.0, intensity: 1.5,
        palette: [c('#000000'), c('#000510'), c('#001133'), c('#003366'), c('#ffaa00'), c('#ffff00'), c('#ffffff'), x],
        p4: 60.0, // Iterations
        p7: 0.9,  // Glow
        p8: 3.0,  // Color Stripe
        p13: -0.7, // Pan X
  ...
  p(TextureType.NEON_RIPPLES, 'Abstract', 'Synthwave Grid', {
    params: {
        scale: 2.0, intensity: 1.5, speed: 0.8,
  ...
  p(TextureType.SPACE_DUST, 'Abstract', 'Nebula Cloud', {
    params: {
        scale: 1.0, intensity: 1.2, speed: 0.2,
        palette: [c('#000000'), c('#110022'), c('#330044'), c('#660066'), c('#ff0088'), c('#00aaff'), x, x],
        p2: 0.5, // Density
        p6: 0.8, // Stars
        p7: 0.6  // Nebula
    },
    environment: { fogEnabled: true, fogColor: '#110022', fogDensity: 0.05, sceneBloom: true, sceneBloomIntensity: 0.5 }
  }),
