      ...
      <>
        <ControlSection title="Animation & Export" icon={Icons.Film} color="#8B5CF6">
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <Slider
                ...
                max={16}
                step={1}
                value={state.spriteSheet.columns}
                onChange={(v) => updateStateGroup("spriteSheet", { columns: v })}
                onCommit={onCommit}
              />
            </div>
            <div>
              ...
              <Slider
                ...
                max={16}
                step={1}
                value={state.spriteSheet.rows}
                onChange={(v) => updateStateGroup("spriteSheet", { rows: v })}
                onCommit={onCommit}
              />
            </div>
          </div>
          <div className="space-y-3">
            <div>
              <Label label="Total Frames" />
              <Slider
                min={1}
                max={64}
                step={1}
                value={state.spriteSheet.totalFrames}
                onChange={(v) => updateStateGroup("spriteSheet", { totalFrames: v })}
                onCommit={onCommit}
              />
            </div>
            <div>
              <Label label="Loop Duration (s)" />
              <Slider
                min={0.1}
                max={5}
                value={state.spriteSheet.duration}
                onChange={(v) => updateStateGroup("spriteSheet", { duration: v })}
                onCommit={onCommit}
              />
            </div>
          </div>
        ...
        >
          <div className="space-y-3">
            ...
            {onDownloadZip && (
              <div className="pt-1">
                <ActionButton
                  ...
                  className="border-accent-primary/30 text-accent-primary hover:bg-accent-primary hover:text-black"
                >
                  {isGenerating ? (
                    <Icons.Loader2 className="animate-spin" size={14} />
                  ) : (
            ...
            <div className="pt-3 border-t border-white/5 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <ActionButton onClick={exportPresets || (() => {})} disabled={!exportPresets}>
                  <Icons.DownloadCloud size={14} /> Save Project
                </ActionButton>
                <input
                  type="file"
                  ref={fileInputRef}
                  ...
                  className="hidden"
                />
                <ActionButton onClick={handleImportClick} disabled={!importPresets}>
                  <Icons.UploadCloud size={14} /> Load Project
                </ActionButton>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-white/5">
              <ActionButton onClick={() => onChangeState({ isFullscreen: true })}>
                <Icons.Maximize size={14} /> Fullscreen
              </ActionButton>
              <button
