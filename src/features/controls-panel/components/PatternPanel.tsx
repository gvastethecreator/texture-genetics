...
export const PatternPanel: React.FC<PatternPanelProps> = memo(({ state, onChangeParams, onChangeState, onCommit, onUpdateAnim, onRandom }) => {
    ...
    const dynamicParams = useMemo(() => {
            ...
            if (label) {
                ...
                params.push(
                    <div key={key} className="animate-in fade-in">
                        ...
                        <Slider 
                            min={range?.min ?? 0} max={range?.max ?? 1} step={range?.step ?? 0.01}
                            // @ts-ignore
        ...
        return params;
    }, [labels, ranges, state.params, state.paramAnimations, onChangeParams, onCommit, onUpdateAnim]);

    return (
        <ControlSection title="Pattern Settings" icon={Icons.Sliders} color="#3B82F6" defaultOpen={true}>
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-1 gap-3">
                    <div>
                        <Label label={labels.scale || 'Scale'} description="Global scale / frequency of the pattern." />
                        <Slider 
                            min={0.1} max={10} 
                    ...
                    <div>
                        <Label label={labels.intensity || 'Intensity'} description="Overall brightness or alpha contrast." />
                        <Slider 
                            min={0} max={3} 
                    ...
                    <div>
                        <Label label={labels.factor || 'Factor'} description="A primary modifier (e.g. thickness, threshold)." />
                        <Slider 
                            min={0} max={1} 
                ...
                <div className="flex items-end gap-2 p-2 bg-black/20 rounded border border-white/5 relative overflow-hidden">
                    <div className="flex-1">
                        {state.animate ? (
                             ...
                             <div className="animate-in fade-in duration-300">
                                ...
                                <Slider min={0} max={100} step={0.01} value={state.time % 100} onChange={(v) => onChangeState({ time: v })} />
                             </div>
                        )}
                    </div>
                    <button
                    ...
                    >
                        {state.animate ? <Icons.Pause size={12} fill="currentColor" /> : <Icons.Play size={12} fill="currentColor" />}
                    </button>
                </div>
            </div>
        </ControlSection>
