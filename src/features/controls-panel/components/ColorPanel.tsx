...
export const ColorPanel: React.FC<ColorPanelProps> = memo(({ state, onChangeParams, updateStateGroup, updateBalance, onCommit, onUpdateAnim, onRandom }) => {
    ...
    const tones: Array<'shadows' | 'midtones' | 'highlights'> = ['shadows', 'midtones', 'highlights'];
    ...
    return (
        <ControlSection title="Color Palette" icon={Icons.Palette} color="#F59E0B">
            ...
            <div className="pt-2 border-t border-white/5 mt-2">
                ...
                <div className="flex gap-2 items-center">
                    <input type="file" accept=".svg,.png,.jpg" onChange={handleMaskUpload} className="hidden" ref={fileInputRef} />
                    <ActionButton onClick={() => fileInputRef.current?.click()}>
                        <Icons.Upload size={12} /> {state.imageAlpha.maskTexture ? "Replace" : "Upload"}
                    </ActionButton>
                    {state.imageAlpha.maskTexture && (
                        <div className="w-8 h-8 rounded border border-gray-600 bg-gray-800 flex items-center justify-center overflow-hidden relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                            <img src={state.imageAlpha.maskTexture} className="w-full h-full object-contain" alt="Mask" />
                ...
                {state.imageAlpha.maskTexture && (
                    <div className="mt-2">
                        <Toggle label="Enable Custom Mask" checked={state.imageAlpha.maskEnabled} onChange={(v) => updateStateGroup('imageAlpha', { maskEnabled: v })} onCommit={onCommit} />
                    </div>
                )}
            </div>
        </ControlSection>
