...
import { TextureType, AppState } from '../../../core/types/types';
...
export const PatternLibrary: React.FC<PatternLibraryProps> = memo(({ currentType, onSelect, onRandom }) => {
    return (
        <div className="mb-4 space-y-1 border border-border/50 rounded-lg overflow-hidden">
            <div className="bg-surface px-3 py-2 border-b border-border/50 flex justify-between items-center">
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Pattern Library</span>
                <button 
                    onClick={onRandom}
            ...
            {Object.entries(TEXTURE_CATEGORIES).map(([catName, data]) => {
                ...
                return (
                    <div key={catName} className="relative">
                        ...
                        >
                            {Icon && <Icon size={14} style={{ color: isActiveCategory ? data.color : 'inherit', opacity: isActiveCategory ? 1 : 0.7 }} />}
                            <span className="text-xs font-medium">{catName}</span>
                            {isActiveCategory && <div className="ml-auto w-1 h-1 rounded-full bg-accent-primary" />}
                        </button>
