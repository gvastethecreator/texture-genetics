    ...
    return (
      <ControlSection title="Material & Depth" icon={Icons.BoxSelect} color="#10B981">
        ...
        <div className="space-y-3 pt-3 border-t border-white/5">
          ...
          <Toggle
            label="Enable Bevel (3D Shapes)"
            checked={state.geometryConfig.bevelEnabled}
            onChange={(v) => updateStateGroup("geometryConfig", { bevelEnabled: v })}
            onCommit={onCommit}
          />
          {state.geometryConfig.bevelEnabled && (
            <div className="mt-2 pl-2 border-l-2 border-accent-primary/20 space-y-2 animate-in fade-in">
              <div>
                <Label label="Bevel Thickness" />
                <Slider
                  min={0.01}
                  max={0.2}
                  step={0.01}
                  value={state.geometryConfig.bevelThickness}
                  onChange={(v) => updateStateGroup("geometryConfig", { bevelThickness: v })}
                  onCommit={onCommit}
                />
              </div>
              <div>
                <Label label="Bevel Size" />
                <Slider
                  min={0.01}
                  max={0.2}
                  step={0.01}
                  value={state.geometryConfig.bevelSize}
                  onChange={(v) => updateStateGroup("geometryConfig", { bevelSize: v })}
                  onCommit={onCommit}
                />
              </div>
              <div>
                <Label label="Bevel Segments" />
                <Slider
                  min={1}
                  max={10}
                  step={1}
                  value={state.geometryConfig.bevelSegments}
                  onChange={(v) => updateStateGroup("geometryConfig", { bevelSegments: v })}
                  onCommit={onCommit}
                />
              </div>
            </div>
          ...
          <div className="mt-2 pl-2 space-y-2">
            <div>
              <Label label="Rounding / Radius" />
              <Slider
                min={0.0}
                max={1.0}
                step={0.01}
                value={state.geometryConfig.rounding}
                onChange={(v) => updateStateGroup("geometryConfig", { rounding: v })}
                onCommit={onCommit}
              />
            </div>
            <div>
              <Label label="Smoothness (Segments)" />
              <Slider
                min={8}
                max={128}
                step={1}
                value={state.geometryConfig.smoothness}
                onChange={(v) => updateStateGroup("geometryConfig", { smoothness: v })}
                onCommit={onCommit}
              />
            </div>
          </div>
        ...
        <div className="space-y-3 pt-3 border-t border-white/5">
          <Toggle
            ...
            checked={state.normalMap.enabled}
            onChange={(v) => updateStateGroup("normalMap", { enabled: v })}
            onCommit={onCommit}
          />
          {state.normalMap.enabled && (
            <div className="mt-2 mb-4 pl-2 border-l-2 border-accent-primary/20 space-y-2 animate-in fade-in">
              <div>
                <Label
                  label="Strength"
                  description="Controls the intensity/depth of the normal map bumps."
                />
                <Slider
                  min={0.1}
                  max={5.0}
              ...
              <div>
                <Label label="Smoothness" />
                <Slider
                  min={0.0}
                  max={1.0}
                  step={0.01}
                  value={state.normalMap.smoothness}
                  onChange={(v) => updateStateGroup("normalMap", { smoothness: v })}
                  onCommit={onCommit}
                />
              </div>
              <Toggle
                label="Invert Y"
                checked={state.normalMap.invert}
                onChange={(v) => updateStateGroup("normalMap", { invert: v })}
                onCommit={onCommit}
              />
            </div>
          )}
        ...
        <div className="space-y-3 pt-3 border-t border-white/5">
          ...
          <Toggle
            label="Enable Holo Effect"
            checked={state.environment.holographic}
            onChange={(v) => updateStateGroup("environment", { holographic: v })}
            onCommit={onCommit}
          />
          {state.environment.holographic && (
            <div className="mt-2 pl-2 border-l-2 border-accent-primary/20 animate-in fade-in">
              <Label label="Foil Strength" />
              <Slider
                min={0}
                max={2.0}
                step={0.1}
                value={state.environment.holoStrength}
                onChange={(v) => updateStateGroup("environment", { holoStrength: v })}
                onCommit={onCommit}
              />
            </div>
          )}
          ...
          <div className="space-y-2 pl-2">
            <div>
              ...
              <Slider
                ...
                max={2.0}
                step={0.1}
                value={state.displacement.strength}
            ...
            <div>
              ...
              <Slider
                ...
                max={1.0}
                step={0.01}
                value={state.displacement.bias}
                onChange={(v) => updateStateGroup("displacement", { bias: v })}
                onCommit={onCommit}
              />
            </div>
          </div>
        ...
        <div className="space-y-3 pt-3 border-t border-white/5">
          <Toggle
            ...
            onChange={(v) => updateStateGroup("ao", { enabled: v })}
            onCommit={onCommit}
          />
          {state.ao.enabled && (
            <div className="mt-2 pl-2 border-l-2 border-accent-primary/20 space-y-2 animate-in fade-in">
