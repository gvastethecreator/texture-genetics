...
export const StickerGizmo: React.FC<StickerGizmoProps> = ({ state, updateState, visible, setControlsEnabled }) => {
    ...
    useEffect(() => { stickerRef.current = state.sticker; }, [state.sticker]);
    
    // Only show if enabled, gizmo explicitly on, AND a texture is loaded
    const shouldShow = visible && state.sticker.enabled && state.sticker.gizmoVisible && !!state.sticker.texture;
        ...
        if (shouldShow) {
            ...
            const handleKeyDown = (event: KeyboardEvent) => {
                ...
                switch(event.key.toLowerCase()) {
                    case 't': transformRef.current.setMode('translate'); break;
                    case 'r': transformRef.current.setMode('rotate'); break;
                    case 's': transformRef.current.setMode('scale'); break;
                }
            };
    ...
    }, [state.sticker.posX, state.sticker.posY, state.sticker.scale, state.sticker.rotation, shouldShow]);
