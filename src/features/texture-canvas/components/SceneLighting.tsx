...
import { supportsWebGLOnlyAddons } from "../../../lib/three/rendererCapabilities";
...
export const SceneLighting: React.FC<SceneLightingProps> = memo(({ appState }) => {
  ...
  const supportsWebGlOnlyDrei = React.useMemo(() => {
    return supportsWebGLOnlyAddons(gl as CanvasRenderer);
  }, [gl]);
  ...
  return (
    <>
      ...
      <directionalLight position={[-5, 0, 5]} intensity={lightIntensity * 0.3} color="#dbeafe" />
