...
export const CameraRig: React.FC<CameraRigProps> = ({ appState, controlsHandle, onZoomChange, updateState, enabled = true }) => {
    ...
    const resetCamera = useCallback(() => {
        const is3D = appState.geometry !== GeometryType.PLANE;
        ...
        if (camera instanceof THREE.PerspectiveCamera) {
            ...
            if (isTiling) {
                ...
                const targetOccupancy = is3D ? 0.65 : 0.90; 
    ...
    }, [appState.geometry, appState.tileMode, appState.tilingPreview, camera, onZoomChange, invalidate, updateState]);
            ...
            controlsHandle.current = {
                ...
                setView: (view: 'front' | 'top' | 'left' | 'right' | 'bottom' | 'back' | 'isometric') => {
                    ...
                    switch (view) {
                        ...
                        case 'top': camera.position.set(target.x, target.y + dist, target.z); break;
                        case 'bottom': camera.position.set(target.x, target.y - dist, target.z); break;
