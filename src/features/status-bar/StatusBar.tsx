...
const Node: React.FC<{
  ...
  subLabel?: string;
  tooltip?: string;
}> = ({ icon: Icon, label, color = "gray", subLabel, tooltip }) => {
  const nodeContent = (
    <div className="flex items-center gap-2.5 px-3 py-1.5 bg-[#151515] border border-white/5 rounded-lg shadow-tactile min-w-fit group hover:bg-[#202020] hover:border-white/10 transition-all cursor-default relative overflow-hidden">
      ...
      <div className="flex flex-col">
        <span className="text-[10px] font-bold text-gray-200 leading-tight whitespace-nowrap tracking-wide">
          {label}
        </span>
        {subLabel && (
          <span className="text-[8px] text-gray-500 leading-tight font-mono uppercase">
            {subLabel}
          </span>
        )}
      </div>
  ...
  if (tooltip) {
    return <PortalTooltip label={tooltip}>{nodeContent}</PortalTooltip>;
  }
  return nodeContent;
};
...
export const StatusBar: React.FC<StatusBarProps> = ({ state, renderer }) => {
        ...
        <div key="fx" className="flex -space-x-2 pl-1">
          {activeEffects.map((fx, i) => (
            <PortalTooltip key={i} label={fx.l}>
              <div
                className="w-7 h-7 rounded-full bg-[#151515] border-2 border-[#2a2a2a] flex items-center justify-center text-[8px] font-bold text-gray-300 z-10 hover:z-20 hover:scale-110 hover:border-white transition-all shadow-lg cursor-help"
                style={{ color: fx.c }}
              >
                {fx.l[0]}
              </div>
  ...
  return (
    <div className="h-12 border-t border-white/5 bg-[#080808] flex items-center justify-between px-6 select-none relative z-20 shadow-[0_-4px_20px_rgba(0,0,0,0.5)]">
      ...
      >
        <span className="text-[9px] font-black text-gray-700 mr-3 uppercase tracking-[0.2em] hidden md:block">
          Signal Flow
        </span>
        {recipeNodes}
      </div>
