...
export const CodeViewerModal: React.FC<CodeViewerModalProps> = ({ isOpen, onClose, state }) => {
    ...
    const highlightCode = useCallback((src: string) => {
        ...
        const tokenPattern = /(<[^>]*>)|(["'`])(?:(?!\2)[^\\]|\\.)*?\2|\b(?:const|let|var|function|return|if|else|for|while|new|import|from|async|await|true|false|null|undefined|typeof|class|this)\b|\b(?:THREE|WebGLRenderer|Scene|OrthographicCamera|PlaneGeometry|Mesh|ShaderMaterial|Vector2|Vector3|Color|SRGBColorSpace|GLSL3|requestAnimationFrame)\b|\b\d+(?:\.\d+)?\b/g;
