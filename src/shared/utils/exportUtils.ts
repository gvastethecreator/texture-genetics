 ...
 */
export const generateStandaloneHtml = (state: AppState): string => {
    ...
    // Construct Palette JS String
    const paletteColorsJS = activeColors.map(c => `new THREE.Color("${c.color}")`).join(',');
    const fillerCount = 8 - activeColors.length;
    const fillerJS = fillerCount > 0 ? ',' + new Array(fillerCount).fill('new THREE.Color(0x000000)').join(',') : '';
    ...
    // Enforce WebGL 2 Context for GLSL 3.0 Support
    ...
    const uniforms={
        ...
        // Post Process
        u_applyToMap:{value:true}, // Always bake for export
        ...
        // Sticker (Disabled in Standalone HTML by default unless dataURI embedded, keeping simpler for now)
        ...
        // Base Texture (Disabled in Standalone by default)
    ...
    const material=new THREE.ShaderMaterial({
        ...
        fragmentShader:\`${frag}\`,
        transparent:true,
        glslVersion: THREE.GLSL3 // WebGL 2 / GLSL 3.0 ES
    });
