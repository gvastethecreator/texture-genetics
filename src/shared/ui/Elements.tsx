...
export const PortalTooltip: React.FC<{ label: string; children: React.ReactElement }> = ({ label, children }) => {
        ...
        <>
            ...
            {isVisible && createPortal(
                <div
                    ...
                    style={{ top: pos.top, left: pos.left, boxShadow: '0 4px 12px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.1)' }}
                >
                    {label}
...
export const Label: React.FC<{ label: string; value?: string | number; description?: string }> = memo(({ label, description }) => (
    <div className="flex justify-between items-center mb-1.5 group relative">
        <span className="text-[11px] font-bold text-gray-400 font-sans tracking-wide cursor-help border-b border-dashed border-transparent group-hover:border-gray-600 transition-colors select-none drop-shadow-sm" title={description}>
            {label}
        </span>
    </div>
));
...
export const Slider: React.FC<SliderProps> = memo(({ value, min, max, step = 0.1, onChange, onCommit, animConfig, onAnimChange }) => {
    ...
    return (
        <div className="relative">
            <div className="flex items-center gap-3 h-8 group/slider">
                ...
                >
                    <input
                        ...
                        onChange={handleSliderChange} onMouseUp={onCommit} onTouchEnd={onCommit}
                        disabled={!!isAnimated}
            ...
            {showAnimPanel && onAnimChange && popoverPos && createPortal(
                <>
                    ...
                    >
                        <div className="flex justify-between items-center mb-3 pb-2 border-b border-white/5">
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider text-gradient">Wave Function</span>
                            <Toggle label="" checked={!!animConfig?.enabled} onChange={toggleAnim} onCommit={onCommit} />
                        </div>

                        {animConfig?.enabled && (
                            <div className="space-y-3">
                                <div>
                                    <span className="text-[9px] text-gray-500 uppercase block mb-1.5 font-bold">Waveform</span>
                                    <div className="flex gap-1 bg-black/40 p-1 rounded-lg shadow-inner">
                                        {[WaveType.SINE, WaveType.COSINE, WaveType.TRIANGLE, WaveType.SAWTOOTH, WaveType.NOISE].map(t => (
                                            <button
                                                key={t}
                                                type="button"
                                            ...
                                            >
                                                ...
                                                {t === WaveType.COSINE && <Icons.Activity size={10} className="scale-x-[-1]" />}
                                ...
                                <div>
                                    <div className="flex justify-between mb-1">
                                        <span className="text-[9px] text-gray-500 uppercase font-bold">Frequency</span>
                                        <span className="text-[9px] font-mono text-purple-400">{animConfig.speed.toFixed(1)}Hz</span>
                                    </div>
                                    <input
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        <span className="text-[9px] text-gray-500 uppercase block mb-1 font-bold">Min</span>
                                        <input
                                            type="number"
                                    ...
                                    <div>
                                        <span className="text-[9px] text-gray-500 uppercase block mb-1 font-bold">Max</span>
                                        <input
                                            type="number"
...
export const Toggle: React.FC<{ label: string; checked: boolean; onChange: (v: boolean) => void; onCommit?: () => void }> = memo(({ label, checked, onChange, onCommit }) => {
    const handleClick = () => {
        onChange(!checked);
    ...
    return (
        ...
        >
            {label && <span className="text-[11px] font-bold text-gray-400 group-hover:text-gray-200 transition-colors tracking-wide">{label}</span>}

            {/* Track */}
            <div className={`w-9 h-5 rounded-full transition-all duration-300 ease-spring relative shadow-inner border border-white/5 ${checked ? 'bg-gradient-to-r from-gray-200 to-white border-white/50' : 'bg-[#0a0a0a] border-white/10'}`}>
                {/* Thumb */}
                <div
...
export const ActionButton: React.FC<{ onClick: () => void; children: React.ReactNode; primary?: boolean; disabled?: boolean; className?: string }> = memo(({ onClick, children, primary, disabled, className = '' }) => {

    const baseStyle = "w-full py-2.5 px-4 rounded-lg font-bold text-[11px] uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-100 ease-out select-none disabled:opacity-50 disabled:cursor-not-allowed";

    // Tactile 3D transform logic
    const activeState = "active:translate-y-[1px] active:shadow-tactile-active active:scale-[0.99]";

    // Gradients & Shadows
    const primaryStyle = "bg-metal-gradient text-white border-t border-white/20 border-b border-black/50 shadow-tactile hover:bg-metal-gradient-hover hover:shadow-tactile-hover hover:text-white hover:border-white/30";

    const secondaryStyle = "bg-[#151515] text-gray-400 border border-white/5 shadow-tactile hover:bg-[#202020] hover:text-white hover:border-white/10 hover:shadow-tactile-hover";

    const highlightedStyle = "bg-gradient-to-b from-gray-200 to-gray-400 text-black border-t border-white/80 border-b border-gray-500 shadow-glow-sm hover:shadow-glow-md hover:from-white hover:to-gray-300";
...
export const Select: React.FC<{ label: string; value: string; options: { label: string; value: string }[]; onChange: (v: string) => void }> = memo(({ label, value, options, onChange }) => (
...
export const ControlSection: React.FC<{
    ...
    return (
        <div className="border border-white/5 rounded-lg overflow-hidden bg-[#0a0a0a] shadow-sm mb-3 transition-all duration-300">
            ...
            >
                <div className="flex items-center gap-2.5">
                    ...
                    <div
                        ...
                        style={{ backgroundColor: color, background: `linear-gradient(135deg, ${color} 0%, #000000 150%)` }}
                    >
                        <Icon size={12} className="text-white mix-blend-overlay" />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 group-hover:text-white transition-colors text-shadow-sm">{title}</span>
                </div>
                <div className={`text-gray-600 transition-transform duration-300 ${isOpen ? 'rotate-180 text-gray-400' : 'rotate-0'}`}>
            ...
            >
                <div className="p-3 border-t border-white/5 bg-[#050505] shadow-inner">
                    {children}
                </div>
            </div>
        </div>
