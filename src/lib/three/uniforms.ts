...
const getPatternUniforms = (state: AppState) => ({
    // SAFETY CLAMP: Ensure scale is never exactly 0.0 to prevent division by zero
    ...
    // Palette Arrays (Max 8)
    u_palette: { value: new Array(8).fill(0).map(() => new THREE.Vector3()) },
    u_paletteCount: { value: 2 },
...
export const createUniformsFromState = (state: AppState, maskTexture: THREE.Texture | null = null, baseTexture: THREE.Texture | null = null, stickerTexture: THREE.Texture | null = null) => {
    const baseTex = state.baseTexture || { enabled: false, opacity: 1.0, blendMode: 0, effectType: BaseEffectType.NONE, effectStrength: 0.5 };
    ...
    const paletteUniformValue = new Array(8).fill(null).map((_, i) => {
...
export const updatePatternUniforms = (uniforms: Record<string, THREE.IUniform>, state: AppState) => {
...
export const updateBlendingUniforms = (uniforms: Record<string, THREE.IUniform>, state: AppState) => {
...
export const updateTransformUniforms = (uniforms: Record<string, THREE.IUniform>, state: AppState) => {
...
export const updatePostProcessUniforms = (uniforms: Record<string, THREE.IUniform>, state: AppState) => {
    ...
    if (uniforms.u_edgeColor.value.getHexString() !== state.postProcess.edgeColor.replace('#', '').toLowerCase()) {
        uniforms.u_edgeColor.value.setStyle(state.postProcess.edgeColor);
    }
};

export const updateMaterialUniforms = (uniforms: Record<string, THREE.IUniform>, state: AppState) => {
...
export const updateColorBalanceUniforms = (uniforms: Record<string, THREE.IUniform>, state: AppState) => {
    if (state.colorBalance) {
        if (state.colorBalance.shadows) {
            uniforms.u_shadows.value.set(state.colorBalance.shadows.r, state.colorBalance.shadows.g, state.colorBalance.shadows.b);
        }
        if (state.colorBalance.midtones) {
            uniforms.u_midtones.value.set(state.colorBalance.midtones.r, state.colorBalance.midtones.g, state.colorBalance.midtones.b);
        }
        if (state.colorBalance.highlights) {
            uniforms.u_highlights.value.set(state.colorBalance.highlights.r, state.colorBalance.highlights.g, state.colorBalance.highlights.b);
        }
...
export const updateEnvironmentUniforms = (uniforms: Record<string, THREE.IUniform>, state: AppState) => {
...
export const updateTextureUniforms = (uniforms: Record<string, THREE.IUniform>, state: AppState, maskTexture: THREE.Texture | null, baseTexture: THREE.Texture | null, stickerTexture: THREE.Texture | null) => {
    // Alpha/Mask
    ...
    const stickerColor = (state.sticker.color && typeof state.sticker.color === 'string') ? state.sticker.color : '#ffffff';
    // Optimization: avoid string parsing if already match
    if (uniforms.u_stickerColor.value.getHexString() !== stickerColor.replace('#', '').toLowerCase()) {
...
export const updateUniformsFromState = (uniforms: Record<string, THREE.IUniform>, state: AppState, maskTexture: THREE.Texture | null = null, baseTexture: THREE.Texture | null = null, stickerTexture: THREE.Texture | null = null) => {
