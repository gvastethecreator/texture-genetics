...
export const projectRendererUniforms = (state: AppState): RendererUniformProjection => {
  ...
  return {
    ...
    interaction: {
      ...
      u_stickerEnabled: state.sticker.enabled,
      u_stickerOpacity: state.sticker.opacity,
      u_stickerBlendMode: state.sticker.blendMode,
      u_stickerPos: [state.sticker.posX, state.sticker.posY],
      u_stickerScale: Math.max(0.01, state.sticker.scale),
      u_stickerRot: state.sticker.rotation * (Math.PI / 180),
      u_stickerColor: state.sticker.color || "#ffffff",
      u_stickerUseColor: state.sticker.useColor,
    },
    environment: {
