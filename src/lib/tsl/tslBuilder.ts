...
import { getAccurateNormal, getAO, getLitColor, getHolographicColor } from "./chunks/lighting";
...
export function buildFragmentGraph(
  ...
  const fragmentFn = Fn(() => {
    ...
    If(u.u_glitch.greaterThan(0.5), () => {
      const noise = random2d(
        floor(st.y.mul(20.0)).add(floor(u.u_time.mul(u.u_glitchSpeed).mul(10.0))),
        float(0.0),
      );
    ...
    if (hasBaseTexture && baseTexture) {
      ...
      if (baseEffect === 4) {
        baseTexUV.addAssign(
          sin(baseTexUV.mul(10.0).add(n.mul(10.0)))
            .mul(0.01)
            .mul(baseEffectStrength),
        );
      }
    ...
    If(viewMode.equal(int(1)).or(viewMode.equal(int(3))), () => {
      const procNormal = getAccurateNormal(
        n,
        u.u_normalStrength,
        u.u_normalEnabled,
        u.u_normalInvert,
 ...
 */
export function createTslMaterial(
  state: AppState,
  textures: TslTextureInputs = {},
): {
