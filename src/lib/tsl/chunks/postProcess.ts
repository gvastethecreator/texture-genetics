  ...
  ([
    ...
    If(float(u_symEnabled).greaterThan(0.5), () => {
      ...
      st.assign(
        vec2(cos(a), sin(a))
          .mul(r)
          .div(max(float(0.01), float(u_symZoom)))
          .add(0.5),
      );
    });
...
export const applyScanlines = Fn(
  ([
    ...
    If(float(u_scanlines).greaterThan(0.5), () => {
      ...
      // Noise
      color.addAssign(
        color.mul(random2d(uv.mul(u_time), float(0.0))).mul(float(u_scanlineIntensity).mul(0.5)),
      );
...
export const applyColorBalance = Fn(
  ([
    ...
    color.assign(
      color
        .sub(0.5)
        .mul(max(float(0.0), float(u_contrast).add(1.0)))
        .add(0.5)
        .add(u_brightness),
    );
