...
export const getHolographicColor = Fn(
    ([baseCol_in, viewDir_in, lightDir_in, normal_in, uv_in, strength_in, lightIntensity_in, scale_in]: any[]) => {
        ...
        If(strength.greaterThan(0.001), () => {
            ...
            const holoDisp = normalize(o).mul(cos(theta)).add(normalize(w).mul(sin(theta))).mul(th);
            const holoNormal = normalize(b.add(holoDisp));

            const n = normalize(mix(normal, holoNormal, 0.8));
            const spectrum = pow(max(dot(viewDir.negate(), reflect(lightDir.negate(), n)), float(0.0)), float(2.0)).toVar();

            // Simplified angle-based iridescence
            const angle = dot(vec3(0.0, 1.0, 0.0), normal);
            const corner = float(1.0).add(cos(float(4.0).mul(angle))).mul(0.5);
...
export const getAO = Fn(([val_in, normal_in, enabled_in, strength_in]: any[]) => {
    const normal = vec3(normal_in);
...
export const fresnelSchlick = Fn(([cosTheta_in, F0_in]: any[]) => {
    ...
    const F0 = vec3(F0_in);
    return F0.add(vec3(1.0).sub(F0).mul(pow(clamp(float(1.0).sub(cosTheta), 0.0, 1.0), float(5.0))));
});
...
export const getLitColor = Fn(
    ([
        ...
        const denominator = float(4.0).mul(max(dot(N, V), float(0.0))).mul(max(dot(N, L), float(0.0))).add(0.0001);
        ...
        const NdotL = max(dot(N, L), float(0.0));
        const directLighting = kD.mul(albedo).div(PI).add(vec3(specular).mul(0.8)).mul(NdotL).mul(lightIntensity);
        ...
        const ambient = kD_ambient.mul(diffuseIBL).add(envColor.mul(F_ambient).mul(0.5)).mul(ao).mul(0.5);
