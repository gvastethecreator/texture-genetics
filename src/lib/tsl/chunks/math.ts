...
export const blendApply = /*@__PURE__*/ Fn(([base_in, blend_in, mode_in]: [any, any, any]) => {
    ...
    // Add
    If(mode.equal(1), () => { result.assign(min(base.add(blend), 1.0)); });
    // Multiply
    If(mode.equal(2), () => { result.assign(base.mul(blend)); });
    // Screen
    If(mode.equal(3), () => { result.assign(float(1).sub(float(1).sub(base).mul(float(1).sub(blend)))); });
    // Overlay
    If(mode.equal(4), () => {
    ...
    If(mode.equal(5), () => {
        result.assign(select(
            blend.lessThan(0.5),
            base.mul(blend).mul(2).add(base.mul(base).mul(float(1).sub(blend.mul(2)))),
            sqrt(base).mul(blend.mul(2).sub(1)).add(base.mul(2).mul(float(1).sub(blend)))
        ));
    });
    // Difference
    If(mode.equal(6), () => { result.assign(abs(base.sub(blend))); });

    return result;
});

/** Vec3 blend modes */
export const applyBlendModeVec3 = /*@__PURE__*/ Fn(([base_in, blend_in, mode_in]: [any, any, any]) => {
    const base = vec3(base_in);
    const blend = vec3(blend_in);
    const mode = int(mode_in);
    const result = vec3(blend).toVar();

    If(mode.equal(1), () => { result.assign(min(base.add(blend), vec3(1))); });
    If(mode.equal(2), () => { result.assign(base.mul(blend)); });
    If(mode.equal(3), () => { result.assign(vec3(1).sub(vec3(1).sub(base).mul(vec3(1).sub(blend)))); });
    If(mode.equal(4), () => {
        result.assign(mix(
            base.mul(blend).mul(2),
...
export const getPaletteColor = /*@__PURE__*/ Fn(([t_in, palette_in, count_in]: [any, any, any]) => {
    ...
    If(index.equal(0), () => { c1.assign(palette_in.element(0)); c2.assign(palette_in.element(1)); });
    If(index.equal(1), () => { c1.assign(palette_in.element(1)); c2.assign(palette_in.element(2)); });
    If(index.equal(2), () => { c1.assign(palette_in.element(2)); c2.assign(palette_in.element(3)); });
    If(index.equal(3), () => { c1.assign(palette_in.element(3)); c2.assign(palette_in.element(4)); });
    If(index.equal(4), () => { c1.assign(palette_in.element(4)); c2.assign(palette_in.element(5)); });
    If(index.equal(5), () => { c1.assign(palette_in.element(5)); c2.assign(palette_in.element(6)); });
    If(index.equal(6), () => { c1.assign(palette_in.element(6)); c2.assign(palette_in.element(7)); });
    If(index.greaterThanEqual(7), () => {
        const last = palette_in.element(count.sub(1));
        c1.assign(last); c2.assign(last);
    });
