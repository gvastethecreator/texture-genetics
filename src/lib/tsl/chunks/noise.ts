...
import {
    ...
    mix, step, max, min, clamp, pow, sqrt,
    length, exp, normalize,
    Loop, If, select,
} from 'three/tsl';
...
export const random2d = /*@__PURE__*/ Fn(([st_in, seed_in]: [any, any]) => {
    ...
    return fract(
        sin(dot(st.add(vec2(seed, seed)), vec2(12.9898, 78.233))).mul(43758.5453123)
    );
});
...
export const hash33 = /*@__PURE__*/ Fn(([p_in]: [any]) => {
    const p = fract(vec3(p_in).mul(vec3(0.1031, 0.1030, 0.0973))).toVar();
...
const mod289_v4 = /*@__PURE__*/ Fn(([x_in]: [any]) => {
    const x = vec4(x_in);
    return x.sub(floor(x.mul(1.0 / 289.0)).mul(289.0));
});
...
export const snoise2d = /*@__PURE__*/ Fn(([v_in]: [any]) => {
    ...
    const p = permute_v3(
        permute_v3(i.y.add(vec3(0.0, i1.y, 1.0))).add(i.x).add(vec3(0.0, i1.x, 1.0))
    );
...
export const fbm2d = /*@__PURE__*/ Fn(([st_in, seed_in, detail_in]: [any, any, any]) => {
    ...
    st.assign(vec2(
        st.x.mul(rotC).sub(st.y.mul(rotS)),
        st.x.mul(rotS).add(st.y.mul(rotC))
    ).mul(2).add(100));
...
export const ridgedFBM = /*@__PURE__*/ Fn(([st_in, seed_in, factor_in]: [any, any, any]) => {
    ...
    Loop(5, () => {
        const n = float(1).sub(abs(noise2d(st, seed).mul(2).sub(1))).toVar();
...
export const voronoi2d = /*@__PURE__*/ Fn(([x_in, time_in]: [any, any]) => {
    ...
    Loop({ start: int(-1), end: int(2), type: 'int', condition: '<' }, ({ i: j }) => {
        Loop({ start: int(-1), end: int(2), type: 'int', condition: '<' }, ({ i }) => {
            ...
            o.assign(sin(t.add(o.mul(6.2831))).mul(0.5).add(0.5));
...
/** Smooth voronoi */
export const voronoiSmooth = /*@__PURE__*/ Fn(([uv_in, time_in, factor_in]: [any, any, any]) => {
    const uv = vec2(uv_in);
    const t = float(time_in);
    const factor = float(factor_in);
    const n = floor(uv);
    ...
    Loop({ start: int(-1), end: int(2), type: 'int', condition: '<' }, ({ i }) => {
        Loop({ start: int(-1), end: int(2), type: 'int', condition: '<' }, ({ i: j }) => {
            Loop({ start: int(-1), end: int(2), type: 'int', condition: '<' }, ({ i: k }) => {
                ...
                If(d0.lessThan(d.x), () => {
                    d.w.assign(d.z); d.z.assign(d.y); d.y.assign(d.x); d.x.assign(d0);
                }).ElseIf(d0.lessThan(d.y), () => {
                    d.w.assign(d.z); d.z.assign(d.y); d.y.assign(d0);
                }).ElseIf(d0.lessThan(d.z), () => {
                    d.w.assign(d.z); d.z.assign(d0);
