...
import {
    ...
    smoothstep, atan, exp, If, Loop, log, normalize,
} from 'three/tsl';
...
const noiseVal = (p: any, seed?: any) => _noise2d(p, seed ?? float(0)).mul(0.5).add(0.5);
...
export const scanlines: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const c = vec2(u.u_resolution).y.mul(u.u_scale).mul(0.1).mul(float(1.0).add(float(u.u_p1).mul(2.0)));
...
export const magicCircle: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        v.addAssign(smoothstep(0.02, 0.0, abs(d.sub(float(0.5).add(cos(a.mul(k).add(u.u_time)).mul(0.1))))));
        // Inner ring noise
...
export const binaryMatrix: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const v = step(10.0, y).mul(random2d(i.add(floor(float(u.u_time).mul(5.0))), u.u_seed)).toVar();
        v.mulAssign(smoothstep(20.0, 10.0, y));
        // Highlight glitch
        If(random2d(vec2(uv.y, float(u.u_time)), u.u_seed).greaterThan(float(1.0).sub(float(u.u_factor).mul(0.2).add(float(u.u_p1).mul(0.5)))), () => {
            v.assign(1.0);
        });
...
export const circuit: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        If(float(u.u_p6).greaterThan(0.0).and(v.greaterThan(0.5)), () => {
            const flicker = step(float(1.0).sub(u.u_p6), random2d(i.add(floor(float(u.u_time).mul(5.0))), u.u_seed));
            v.mulAssign(flicker);
        });
        return pow(clamp(v, 0.0, 1.0), float(u.u_intensity));
    })();
...
export const mandelbrot: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        Loop({ start: int(0), end: int(100), type: 'int', condition: '<' }, ({ i }: { i: any }) => {
            If(float(i).greaterThan(maxIter), () => { return; });
            z.assign(vec2(z.x.mul(z.x).sub(z.y.mul(z.y)), float(2.0).mul(z.x).mul(z.y)).add(c));
            If(dot(z, z).greaterThan(4.0), () => { return; });
            iter.addAssign(1.0);
        });
        return pow(clamp(iter.div(maxIter), 0.0, 1.0), float(u.u_intensity)).mul(float(1.0).add(float(u.u_factor).mul(sin(iter))));
    })();
};
...
export const julia: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const c = vec2(-0.8, 0.156).add(
            vec2(sin(float(u.u_time).mul(0.5)), cos(float(u.u_time).mul(0.5))).mul(0.1).mul(u.u_factor)
        ).toVar();
        const iter = float(0.0).toVar();
        const maxIter = float(20.0).add(float(u.u_detail).mul(80.0));
        Loop({ start: int(0), end: int(100), type: 'int', condition: '<' }, ({ i }: { i: any }) => {
...
export const hypnoticRings: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const val = sin(dist.mul(s).add(angle).add(cos(dist.mul(s).mul(u.u_factor))).sub(t)).toVar();
...
export const neonRipples: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        Loop({ start: int(1), end: int(8), type: 'int', condition: '<' }, ({ i }: { i: any }) => {
            const fi = float(i);
            If(fi.greaterThan(layers), () => { return; });
            uv.y.addAssign(
                fi.mul(0.1).div(fi).mul(sin(uv.x.mul(fi).mul(fi).add(t))).mul(sin(uv.y.mul(fi).mul(fi).add(t)))
            );
        });
...
export const blackHole: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        const uv = vec2(st).mul(2.0).sub(1.0).mul(u.u_scale).toVar();
        const len = length(uv);
        const n0 = snoise2d(uv.mul(0.65).add(float(u.u_time).mul(0.5))).mul(0.5).add(0.5);
        ...
        const v0 = float(1.0).div(float(1.0).add(d0.mul(10.0))).toVar();
    ...
    return Fn(() => {
        ...
        Loop({ start: int(0), end: int(60), type: 'int', condition: '<' }, () => {
            ...
            Loop({ start: int(0), end: int(5), type: 'int', condition: '<' }, () => {
                ...
                const vxz = vec2(v.x, v.z).mul(rot2D(float(1.0).add(u.u_factor))).toVar();
                v.x.assign(vxz.x);
                v.z.assign(vxz.y);
            ...
            If(length(p).greaterThan(20.0), () => { return; });
        });
