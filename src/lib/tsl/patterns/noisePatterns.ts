...
import {
  ...
  ridgedFBM,
  voronoi2d,
  voronoiSmooth as _voronoiSmooth,
  worley3d,
  gyroid,
  curlNoise,
  hash22,
...
export const perlinNoise: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    If(float(u.u_p13).notEqual(0.0), () => {
      stv.assign(
        rotate2d(u.u_p13)
          .mul(stv.sub(vec2(0.5).mul(u.u_scale).mul(3.0)))
          .add(vec2(0.5).mul(u.u_scale).mul(3.0)),
      );
    });
    // Domain warp P7/P8
...
export const simplexNoise: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    If(float(u.u_p3).greaterThan(0.0), () => {
      n.assign(
        pow(max(float(0.0), n.mul(0.5).add(0.5)), float(1.0).add(float(u.u_p3).mul(2.0)))
          .mul(2.0)
          .sub(1.0),
      );
    });
    If(float(u.u_p4).greaterThan(0.0), () => {
      n.addAssign(
        snoise3d(vec3(stv.mul(2.0), float(u.u_time).mul(0.3))).mul(float(u.u_p4).mul(0.5)),
      );
    });
...
export const cellularNoise: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    If(float(u.u_factor).greaterThan(0.0), () => {
      dist.assign(
        smoothstep(float(u.u_factor).mul(0.2), 0.0, abs(dist.sub(float(0.5).mul(u.u_factor)))),
      );
    }).Else(() => {
...
export const gradientNoise: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    If(float(u.u_p2).greaterThan(0.0), () => {
      g.assign(
        sin(g.mul(float(u.u_p2).mul(10.0)))
          .mul(0.5)
          .add(0.5),
      );
    });
    If(float(u.u_p12).greaterThan(0.0), () => {
      n.assign(
        sin(n.mul(10.0).add(float(u.u_p12).mul(6.28)))
          .mul(0.5)
          .add(0.5),
      );
    });
...
export const ridgedFractal: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    If(float(u.u_p1).greaterThan(0.0), () => {
      n.mulAssign(float(1.0).add(u.u_p1));
    });
    If(float(u.u_p2).greaterThan(0.0), () => {
...
export const voronoiRocks: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
      ...
      vec2(
        sin(
          p.x
            .mul(6.28)
            .mul(2.0)
            .sub(cos(p.y.mul(6.28).mul(4.0))),
        ),
        sin(
          p.y
            .mul(6.28)
            .mul(2.0)
            .sub(cos(p.x.mul(6.28).mul(4.0))),
        ),
      )
        .mul(0.01)
        .mul(u.u_distortion),
    ...
    If(float(u.u_factor).greaterThan(0.5), () => {
      rock.assign(float(1.0).sub(rock));
    });
  ...
  return Fn(() => {
    ...
    If(float(u.u_factor).lessThan(0.33), () => {
      ...
      If(float(u.u_factor).lessThan(0.66), () => {
        ...
        const denom = float(1.0)
          .div(w.z.sub(w.x).add(0.001))
          .add(float(1.0).div(w.y.sub(w.x).add(0.001)));
...
export const turbulence: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    const f = abs(fbm2d(s, u.u_seed, u.u_detail).mul(2.0).sub(1.0))
      .mul(0.5)
      .toVar();
    s.mulAssign(2.1);
...
export const gyroidPattern: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    d.assign(
      d.mul(float(1.0).sub(float(u.u_factor).mul(0.5))).add(float(u.u_factor).mul(sin(d.mul(5.0)))),
    );
...
export const marble: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    const m = cos(p.x.mul(veins).add(n.mul(distort).mul(u.u_factor)))
      .mul(0.5)
      .add(0.5)
      .toVar();
    If(float(u.u_p1).greaterThan(0.0), () => {
...
export const wood: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    const w = sin(length(p.sub(0.5)).add(n.mul(0.1).mul(u.u_p7)).mul(rings))
      .mul(0.5)
      .add(0.5)
      .toVar();
    If(float(u.u_p4).greaterThan(0.0), () => {
...
export const grunge: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    If(float(u.u_p4).greaterThan(0.0), () => {
      scratches.assign(
        smoothstep(
          float(0.98).sub(float(u.u_p4).mul(0.1)),
          1.0,
          random2d(vec2(p.y, p.x), u.u_seed),
        ),
      );
    });
...
export const holoFoil: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    return pow(
      clamp(v, 0.0, 1.0),
      float(u.u_intensity).mul(float(1.0).sub(float(u.u_factor).mul(0.5))),
    );
  })();
};
...
export const oilPaint: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    const r = vec2(
      fbm2d(
        p.add(q.mul(4.0)).add(vec2(1.7, 9.2)).add(float(u.u_time).mul(0.15)),
        u.u_seed,
        u.u_detail,
      ),
      fbm2d(
        p.add(q.mul(4.0)).add(vec2(8.3, 2.8)).add(float(u.u_time).mul(0.126)),
        u.u_seed,
        u.u_detail,
      ),
    );
...
export const fluidWarp: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    const r = vec2(
      fbm2d(
        p.add(q.mul(4.0)).add(vec2(1.7, 9.2)).add(float(u.u_time).mul(u.u_speed).mul(0.15)),
        u.u_seed,
        u.u_detail,
      ),
      fbm2d(
        p.add(q.mul(4.0)).add(vec2(8.3, 2.8)).add(float(u.u_time).mul(u.u_speed).mul(0.126)),
        u.u_seed,
        u.u_detail,
      ),
    );
