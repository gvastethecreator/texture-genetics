...
export const hyperTunnel: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    Loop({ start: int(0), end: int(80), type: "int", condition: "<" }, ({ i }: { i: any }) => {
      If(float(i).greaterThan(float(40.0).add(float(u.u_p6).mul(40.0))), () => {
        return;
      });
      ...
      const w = float(0.01).add(
        float(0.07).mul(
          abs(max(wave, sqrt(s.mul(s).add(dot(p, p))).sub(1.5)).sub(float(i).div(150.0))),
        ),
      );
      ...
      accum.addAssign(
        float(1.3)
          .div(max(0.001, wScaled))
          .mul(0.002)
          .mul(u.u_intensity)
          .mul(float(1.0).add(u.u_p2)),
      );
      If(d.greaterThan(20.0), () => {
        return;
      });
    });
    // Fog fadeout
...
export const alienBiomass: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    Loop({ start: int(0), end: int(50), type: "int", condition: "<" }, () => {
      ...
      const pxy = vec2(p.x, p.y)
        .mul(rot2D(cos(w).mul(0.1)))
        .toVar();
      p.x.assign(pxy.x);
      p.y.assign(pxy.y);
...
export const cubicSpace: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    const ro = vec3(
      float(u.u_p12).mul(2.0),
      float(u.u_p13).mul(2.0),
      float(-4.0).add(float(u.u_p11).mul(2.0)),
    ).toVar();
    const rd = normalize(vec3(vec2(st).sub(0.5).mul(2.0), 1.0)).toVar();
    const t = float(u.u_time).mul(u.u_speed);
    ...
    Loop({ start: int(0), end: int(40), type: "int", condition: "<" }, ({ i }: { i: any }) => {
      ...
      // Rotate q
      const fi = float(i);
      const qxy = vec2(q.x, q.y)
        .mul(rot2D(t.add(fi.mul(0.1))))
        .toVar();
      q.x.assign(qxy.x);
      q.y.assign(qxy.y);
      const qxz = vec2(q.x, q.z)
        .mul(rot2D(t.mul(0.5)))
        .toVar();
      q.x.assign(qxz.x);
      q.z.assign(qxz.y);
...
export const lowTechTunnel: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    const p2 = vec2(st).sub(0.5).mul(2.0);
    const T = float(u.u_time)
      .mul(u.u_speed)
      .mul(4.0)
      .add(5.0)
      .add(sin(float(u.u_time).mul(0.3)).mul(5.0));
    // Camera path
    const pathZ = T;
    ...
    const Y = cross(X, Z);
    // Camera matrix
    const rd = normalize(X.mul(p2.x).add(Y.mul(p2.y)).add(Z.mul(1.0))).toVar();
    const d = float(0.0).toVar();
    ...
    Loop({ start: int(0), end: int(28), type: "int", condition: "<" }, () => {
      ...
      s.subAssign(
        min(
          length(vec2(ro.x, ro.y).sub(xPath.sub(6.0))),
          length(vec2(ro.x.sub(xPath), ro.y.sub(yPath))),
        ),
      );
      d.addAssign(abs(s).mul(0.3).add(0.01));
      accum.addAssign(float(1.0).div(max(0.001, abs(s))));
      If(d.greaterThan(30.0), () => {
        return;
      });
    });
...
export const octgrams: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    // Rotate ray
    const rxy = vec2(ray.x, ray.y)
      .mul(rot2D(sin(float(u.u_time).mul(0.03)).mul(5.0)))
      .toVar();
    ray.x.assign(rxy.x);
    ray.y.assign(rxy.y);
    const ryz = vec2(ray.y, ray.z)
      .mul(rot2D(sin(float(u.u_time).mul(0.05)).mul(0.2)))
      .toVar();
    ray.y.assign(ryz.x);
    ray.z.assign(ryz.y);
    ...
    Loop({ start: int(0), end: int(99), type: "int", condition: "<" }, ({ i: _i }: { i: any }) => {
      ...
      const b1 = sdBox(posY1.mul(boxScale), vec3(0.4, 0.4, 0.1))
        .div(1.5)
        .negate();
      ...
      const b2 = sdBox(posY2.mul(boxScale), vec3(0.4, 0.4, 0.1))
        .div(1.5)
        .negate();

      const b5 = sdBox(pos.mul(0.5), vec3(0.4, 0.4, 0.1))
        .div(1.5)
        .negate()
        .mul(6.0);
      ...
      ac.addAssign(
        exp(
          dAbs
            .negate()
            .mul(23.0)
            .mul(float(1.0).sub(float(u.u_factor).mul(0.5))),
        ),
      );
...
export const indraNet: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    Loop({ start: int(0), end: int(40), type: "int", condition: "<" }, () => {
      ...
      const connections = min(
        length(vec2(p.x, p.y)),
        min(length(vec2(p.y, p.z)), length(vec2(p.z, p.x))),
      ).sub(float(0.05).add(float(u.u_factor).mul(0.1)));
      const shape = smin(sphere, connections, 0.2);
      d.addAssign(max(0.02, abs(shape)));
      accum.addAssign(
        float(0.02)
          .div(float(0.02).add(abs(shape)))
          .mul(u.u_intensity),
      );
      If(d.greaterThan(20.0), () => {
        return;
...
export const sphericalSpiral: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    If(r.lessThanEqual(1.0), () => {
      ...
      const t = float(u.u_time).mul(u.u_speed);
      // Rotations
      const posxz = vec2(pos.x, pos.z)
        .mul(rot2D(t.add(u.u_p12)))
        .toVar();
      pos.x.assign(posxz.x);
      pos.z.assign(posxz.y);
      const posyz = vec2(pos.y, pos.z)
        .mul(rot2D(t.mul(0.5).add(u.u_p13)))
        .toVar();
...
export const alienCocoon: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    const rd = normalize(vec3(vec2(st).sub(0.5).mul(2.0), 1.0)).toVar();
    // P14 rotation
    const rdxy = vec2(rd.x, rd.y)
      .mul(rot2D(float(u.u_p14)))
      .toVar();
    rd.x.assign(rdxy.x);
    rd.y.assign(rdxy.y);
...
export const volumetricFog: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    const ro = vec3(
      u.u_p12,
      float(1.0).add(u.u_p13),
      float(u.u_time).mul(u.u_speed).add(u.u_p11),
    ).toVar();
    const rd = normalize(vec3(vec2(st).sub(0.5).mul(2.0), 1.0)).toVar();
    rd.y.subAssign(0.3);
    // P14 tilt
    If(float(u.u_p14).abs().greaterThan(0.0), () => {
      const rdyz = vec2(rd.y, rd.z)
        .mul(rot2D(float(u.u_p14)))
        .toVar();
      rd.y.assign(rdyz.x);
      rd.z.assign(rdyz.y);
