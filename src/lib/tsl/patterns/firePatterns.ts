...
export const complexFire: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const fuel = pow(max(float(0.0), xfuel.mul(ypart).mul(float(1.0).add(float(u.u_p10).mul(0.5)))), float(1.0).add(float(u.u_factor).mul(2.0)));
        ...
        If(float(u.u_p6).greaterThan(0.33), () => {
            ...
            n.assign(float(0.5).mul(n).add(float(0.5).mul(n2.mul(0.5).add(0.5))));
        });
        If(float(u.u_p6).greaterThan(0.66), () => {
            const n3 = snoise3d(pAnim.mul(4.0));
            n.assign(float(0.5).mul(n).add(float(0.5).mul(n3.mul(0.5).add(0.5))));
        });
        ...
        If(float(u.u_p4).greaterThan(0.0), () => {
            const smoke = snoise3d(pAnim.mul(0.5).add(vec3(0.0, u.u_time, 0.0))).mul(0.5).add(0.5);
...
export const flame: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const wind = sin(uv.y.mul(10.0).add(float(u.u_time).mul(5.0))).mul(0.05).mul(u.u_p2);
        uv.x.addAssign(wind);
        const n = fbm2d(uv.mul(5.0).mul(u.u_scale).sub(vec2(0, float(u.u_time).mul(2.0))), u.u_seed, u.u_detail);
...
export const fire: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        return pow(clamp(f, 0.0, 1.0), u.u_intensity).mul(float(1.0).add(pow(clamp(f, 0.0, 1.0), 4.0).mul(u.u_factor)));
    })();
};
...
export const solar: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const val = float(0.1).mul(u.u_scale).add(flares).div(max(float(0.01), d.sub(c.mul(0.1).mul(u.u_factor))));
...
export const spark: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const s = float(0.02).div(max(float(0.001), d)).toVar();
        ...
        return pow(clamp(s.mul(float(0.8).add(_noise2d(uv.mul(10.0).add(float(u.u_time).mul(5.0)), u.u_seed).mul(0.2))), 0.0, 1.0), u.u_intensity);
    })();
};
...
export const flash: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const a = atan(uv.y, uv.x);
        v.addAssign(sin(a.mul(20.0).mul(u.u_factor).add(float(u.u_time).mul(10.0))).mul(0.1).mul(v).mul(u.u_factor));
...
export const laser: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        const uv = vec2(st).mul(u.u_scale).toVar();
        If(float(u.u_p2).greaterThan(0.0), () => {
            uv.x.addAssign(sin(uv.y.mul(20.0).add(float(u.u_time).mul(10.0))).mul(0.01).mul(u.u_p2));
        });
        ...
        return clamp(
            b.mul(u.u_intensity).add(core.mul(2.0)).mul(float(1.0).add(sin(uv.x.mul(20.0).sub(float(u.u_time).mul(10.0))).mul(0.2))),
            0.0,
            1.0,
        );
    })();
...
export const flare: TslPatternFn = (st: any, u: TslUniforms) => {
    return Fn(() => {
        ...
        const g = float(0.2).div(max(float(0.001), length(uv))).toVar();
        // Unroll 4 lens ghosts
        g.addAssign(float(0.05).div(max(float(0.001), distance(uv, uv.mul(-0.5).mul(1.0)))).mul(u.u_factor));
        g.addAssign(float(0.05).div(max(float(0.001), distance(uv, uv.mul(-0.5).mul(2.0)))).mul(u.u_factor));
        g.addAssign(float(0.05).div(max(float(0.001), distance(uv, uv.mul(-0.5).mul(3.0)))).mul(u.u_factor));
        g.addAssign(float(0.05).div(max(float(0.001), distance(uv, uv.mul(-0.5).mul(4.0)))).mul(u.u_factor));
