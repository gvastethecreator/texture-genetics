...
import { noise2d as _noise2d, random2d, fbm2d } from "../chunks/noise";
...
export const smoke: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    const uv = vec2(st).mul(u.u_scale);
    const q = vec2(fbm2d(uv, u.u_seed, u.u_detail), fbm2d(uv.add(5.2), u.u_seed, u.u_detail));
    const f = fbm2d(
      uv.add(q.mul(u.u_factor).mul(2.0)).add(float(u.u_time).mul(0.2)),
      u.u_seed,
      u.u_detail,
    )
      .mul(0.5)
...
export const water: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    p.x.addAssign(
      d.div(1.0).mul(sin(float(1.0).mul(3.0).mul(p.y).add(float(u.u_time).mul(spd).mul(u.u_p4)))),
    );
    p.y.addAssign(
      d.div(1.0).mul(cos(float(1.0).mul(3.0).mul(p.x).add(float(u.u_time).mul(spd).mul(u.u_p5)))),
    );
    p.x.addAssign(
      d.div(2.0).mul(sin(float(2.0).mul(3.0).mul(p.y).add(float(u.u_time).mul(spd).mul(u.u_p4)))),
    );
    p.y.addAssign(
      d.div(2.0).mul(cos(float(2.0).mul(3.0).mul(p.x).add(float(u.u_time).mul(spd).mul(u.u_p5)))),
    );
    p.x.addAssign(
      d.div(3.0).mul(sin(float(3.0).mul(3.0).mul(p.y).add(float(u.u_time).mul(spd).mul(u.u_p4)))),
    );
    p.y.addAssign(
      d.div(3.0).mul(cos(float(3.0).mul(3.0).mul(p.x).add(float(u.u_time).mul(spd).mul(u.u_p5)))),
    );
    p.x.addAssign(
      d.div(4.0).mul(sin(float(4.0).mul(3.0).mul(p.y).add(float(u.u_time).mul(spd).mul(u.u_p4)))),
    );
    p.y.addAssign(
      d.div(4.0).mul(cos(float(4.0).mul(3.0).mul(p.x).add(float(u.u_time).mul(spd).mul(u.u_p5)))),
    );
  ...
  return Fn(() => {
    ...
    Loop({ start: int(1), end: int(5), type: "int", condition: "<" }, ({ i }) => {
      ...
      q.x.addAssign(
        float(u.u_time)
          .mul(windX)
          .add(sin(float(u.u_time).add(q.y)).mul(0.5)),
      );
      const f = fract(q);
      const floorQ = floor(q);
      If(
        random2d(floorQ, u.u_seed).greaterThan(float(0.98).sub(float(u.u_factor).mul(0.1))),
        () => {
...
export const rain: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    If(random2d(floorU, u.u_seed).greaterThan(float(0.95).sub(float(u.u_factor).mul(0.2))), () => {
      const f = fract(uv);
      const streak = float(1.0)
        .sub(f.y)
        .mul(smoothstep(0.0, 0.2, f.x).sub(smoothstep(0.8, 1.0, f.x)))
        .toVar();
      If(float(u.u_p6).greaterThan(0.0), () => {
        streak.mulAssign(
          float(1.0).add(float(u.u_p6).mul(sin(float(u.u_time).mul(10.0).add(f.y.mul(10.0))))),
        );
      });
      r.assign(streak);
    });
...
export const bubbles: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    If(random2d(floorU, u.u_seed).greaterThan(float(0.9).sub(float(u.u_factor).mul(0.3))), () => {
      const c = vec2(0.5).add(
        vec2(sin(float(u.u_time).mul(5.0).add(random2d(floorU, u.u_seed).mul(10.0))).mul(0.2), 0.0),
      );
...
export const caustics: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    Loop({ start: int(0), end: int(4), type: "int", condition: "<" }, ({ i }) => {
      ...
      iv.assign(
        uv.add(
          vec2(cos(t2.sub(iv.x)).add(sin(t2.add(iv.y))), sin(t2.sub(iv.y)).add(cos(t2.add(iv.x)))),
        ),
      );
      c.addAssign(
        float(1.0).div(
          max(
            float(0.01),
            length(vec2(uv.x.div(sin(iv.x.add(t2)).div(k)), uv.y.div(cos(iv.y.add(t2)).div(k)))),
          ),
        ),
      );
    });
...
export const inkSplat: TslPatternFn = (st: any, u: TslUniforms) => {
  return Fn(() => {
    ...
    const d = length(uv);
    const r = float(0.5)
      .add(fbm2d(vec2(atan(uv.y, uv.x).mul(5.0), u.u_time), u.u_seed, u.u_detail).mul(0.2))
      .toVar();
    r.addAssign(
      _noise2d(uv.mul(float(5.0).add(float(u.u_factor).mul(10.0))).add(10.0), u.u_seed)
        .mul(0.3)
        .mul(u.u_factor),
    );
    return clamp(
      float(1.0)
        .sub(smoothstep(r.sub(0.1), r.add(0.1), d))
        .mul(u.u_intensity),
      0.0,
      1.0,
    );
